...
class CheckSparseEngine(PatchEngine):
    ...
    def _get_patch_title(self):
        ...
        if _cnt['symbol'] == _cnt['total']:
            ...
            title = 'fix warning for using 0 as NULL'
        elif _cnt['non_ansi'] == _cnt['total']:
            title = 'fix non-ANSI function warning'
        elif _cnt['duplicate'] == _cnt['total']:
            title = 'fix duplicate const warning'
    ...
    def _fix_plain_integer_as_null(self, line):
        ...
        self._execute_shell("sed -i '%ss/return false/return NULL/' %s" % (a[1], self._get_build_path()))
        return True
