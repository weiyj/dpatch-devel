...
class PatchFormater(object):
    ...
    def _guest_old_function_name(self):
        oldfuncname = []
        ...
        for line in self._content.split('\n'):
            ...
            if re.search('\w+\s*\(', line):
                ...
                if len(line) != 0:
                    fun = line.split(' ')[-1]
                    fun = re.sub('\W+', '', fun)
                    if len(fun) != 0 and oldfuncname.count("%s()" % fun) == 0:
                        oldfuncname.append("%s()" % fun)

        return oldfuncname

    def _guest_new_function_name(self):
        newfuncname = []
        ...
        for line in self._content.split('\n'):
            ...
            if re.search('\w+\s*\(', line):
                ...
                if len(line) != 0:
                    fun = line.split(' ')[-1]
                    fun = re.sub('\W+', '', fun)
                    if len(fun) != 0 and newfuncname.count("%s()" % fun) == 0:
                        newfuncname.append("%s()" % fun)
    ...
    def _format_value(self, value):
        if re.search(r'{{[^}]*}}', value):
            ...
            if len(oldfuncs) == 1:
                if re.search(r'{{\s*oldfunction\s*}}', value):
                    value = re.sub(r'{{\s*oldfunction\s*}}', oldfuncs[0], value)
                if re.search(r'{{\s*oldfunction1\s*}}', value):
                    value = re.sub(r'{{\s*oldfunction1\s*}}', oldfuncs[0], value)
                if re.search(r'{{\s*oldfunc\s*}}', value):
                    value = re.sub(r'{{\s*oldfunc\s*}}', oldfuncs[0], value)
                if re.search(r'{{\s*oldfuncname\s*}}', value):
                    value = re.sub(r'{{\s*oldfuncname\s*}}', oldfuncs[0][:-2], value)
                if re.search(r'{{\s*oldfunc1\s*}}', value):
                    value = re.sub(r'{{\s*oldfunc1\s*}}', oldfuncs[0], value)
                if re.search(r'{{\s*oldfuncname1\s*}}', value):
                    value = re.sub(r'{{\s*oldfuncname1\s*}}', oldfuncs[0][:-2], value)
            elif len(oldfuncs) > 1:
                if re.search(r'{{\s*oldfunction\s*}}', value):
                    value = re.sub(r'{{\s*oldfunction\s*}}', oldfuncs[0], value)
                if re.search(r'{{\s*oldfunc\s*}}', value):
                ...
                if re.search(r'{{\s*oldfuncname1\s*}}', value):
                    value = re.sub(r'{{\s*oldfuncname1\s*}}', oldfuncs[0][:-2], value)
                if re.search(r'{{\s*oldfunction2\s*}}', value):
                    value = re.sub(r'{{\s*oldfunction2\s*}}', oldfuncs[1], value)
                if re.search(r'{{\s*oldfunc2\s*}}', value):
                    value = re.sub(r'{{\s*oldfunc2\s*}}', oldfuncs[1], value)
                if re.search(r'{{\s*oldfuncname2\s*}}', value):
                    value = re.sub(r'{{\s*oldfuncname2\s*}}', oldfuncs[1][:-2], value)
            else:
                # keep format
                pass
                #if re.search(r'{{\s*oldfunction\s*}}', value):
                #    value = re.sub(r'{{\s*oldfunction\s*}}', '', value)

            newfuncs = self._guest_old_function_name()
            if len(newfuncs) == 1:
                if re.search(r'{{\s*newfunction\s*}}', value):
                    value = re.sub(r'{{\s*newfunction\s*}}', newfuncs[0], value)
                if re.search(r'{{\s*newfunc\s*}}', value):
                    value = re.sub(r'{{\s*newfunc\s*}}', newfuncs[0], value)
                if re.search(r'{{\s*newfuncname\s*}}', value):
                    value = re.sub(r'{{\s*newfuncname\s*}}', newfuncs[0][:-2], value)
                if re.search(r'{{\s*newfunction1\s*}}', value):
                    value = re.sub(r'{{\s*newfunction1\s*}}', newfuncs[0], value)
                if re.search(r'{{\s*newfunc1\s*}}', value):
                    value = re.sub(r'{{\s*newfunc1\s*}}', newfuncs[0], value)
                if re.search(r'{{\s*newfuncname1\s*}}', value):
                    value = re.sub(r'{{\s*newfuncname1\s*}}', newfuncs[0][:-2], value)
            elif len(newfuncs) > 1:
                if re.search(r'{{\s*newfunction\s*}}', value):
                    value = re.sub(r'{{\s*newfunction\s*}}', newfuncs[0], value)
                if re.search(r'{{\s*newfunc\s*}}', value):
                ...
                if re.search(r'{{\s*newfuncname1\s*}}', value):
                    value = re.sub(r'{{\s*newfuncname1\s*}}', newfuncs[0][:-2], value)
                if re.search(r'{{\s*newfunction2\s*}}', value):
                    value = re.sub(r'{{\s*newfunction2\s*}}', newfuncs[1], value)
                if re.search(r'{{\s*newfunc2\s*}}', value):
                    value = re.sub(r'{{\s*newfunc2\s*}}', newfuncs[1], value)
                if re.search(r'{{\s*newfuncname2\s*}}', value):
                    value = re.sub(r'{{\s*newfuncname2\s*}}', newfuncs[1][:-2], value)
    ...
    def get_module(self):
        ...
        if not os.path.isdir(self._fullpath()):
            mnames = FileModule.objects.filter(file = self._fname)
            if len(mnames) != 0:
