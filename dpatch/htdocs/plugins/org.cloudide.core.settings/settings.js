define(function(require, exports, module) {
    main.consumes = ["app", "ui", "Plugin", "api", "fs", "utils"];
    ...
    function main(options, imports, register) {
        ...
        Plugin.instance.settings = plugin;

        // We'll have a lot of listeners, so upping the limit
        emit.setMaxListeners(10000);
        
        var resetSettings = options.reset || app.location.match(/reset=([\w\|]*)/) && RegExp.$1;
        var develMode = app.location.indexOf("devel=1") > -1;
        var debugMode = app.location.indexOf("debug=2") > -1;
        ...
        var INTERVAL = 1000;
        var PATH = {
            "project" : app.toInternalPath(options.projectConfigPath || "/settings") + "/project.settings",
            "user"    : app.toInternalPath(options.userConfigPath || "/settings") + "/user.settings",
            "state"   : app.toInternalPath(options.stateConfigFilePath || (options.stateConfigPath || "/settings") + "/state.settings")
        };
        ...
        function loadSettings(json) {
            if (!json) {
                ...
                // Load from parsed settings in the index file
                    ...
                    if (debugMode)
                        json.state = localStorage["debugState" + app.projectName];
        ...
        function saveModel(forceSync) {
            if (app.readonly || !app.has(app.NETWORK)) return;
            ...
            if (model.loaded && !testing) {
                KEYS.forEach(function(type) {
                    ...
                    // Get XML string
                    var json = utils.stableStringify(node, 0, "    ");
                    ...
                    if (debugMode && type == "state") {
                        localStorage["debugState" + app.projectName] = json;
                        return;
                    }
        ...
        if (app.connected || options.settings) 
            loadSettings();
        else
            app.once("connect", loadSettings);
